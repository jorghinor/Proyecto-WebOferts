            ...
            <aside>
              ...
              <div class="widget categories">
                ...
                <ul class="categories-list">
                  <li v-for="cate in categoriasList" :key="cate.id" :style="getCategoryItemStyle(cate)">
          ...
          <div class="col-lg-9 col-md-12 col-xs-12 page-content">
            <div class="product-filter">
              <div class="short-name">
                <span>Mostrando {{ anunciosFiltrados.length }} anuncios en <strong>{{ categoria.cname }}</strong></span>
              </div>
              <div class="Show-item">
                    ...
                    >
                      ...
                      <div class="product-content">
                        <h3 class="product-title text-truncate-2">
                          <a class="wf-focusable" :href="getLink(anuncio.id)">{{anuncio.titulo}}</a>
                        </h3>
                        <div class="business-header">
                          ...
                          <div class="business-copy">
                            <p class="business-name text-truncate">{{anuncio.negocio.nnegocio}}</p>
                        ...
                        <span class="price" v-if="anuncio.producto">{{anuncio.producto.precio_regular}} Bs.-</span>
            ...
            <!-- Paginación -->
            <div class="pagination-bar" v-if="anunciosData.last_page > 1">
               <nav>
                <ul class="pagination justify-content-center">
                  <li class="page-item" :class="{ disabled: !anunciosData.prev_page_url }">
                    <a class="btn page-link" :href="anunciosData.prev_page_url" v-if="anunciosData.prev_page_url">Previo</a>
                    <span class="btn page-link" v-else>Previo</span>
                  </li>
                  <li class="page-item active"><span class="btn page-link">{{ anunciosData.current_page }}</span></li>
                  <li class="page-item" :class="{ disabled: !anunciosData.next_page_url }">
                    <a class="btn page-link" :href="anunciosData.next_page_url" v-if="anunciosData.next_page_url">Sig</a>
                    <span class="btn page-link" v-else>Sig</span>
                  </li>
                </ul>
               </nav>
            </div>
...
export default {
  ...
  data() {
    return {
      categoriasList: [],
      sidebarSearch: "",
      // We use a local copy to handle pagination updates if needed,
      // though initially it comes from props.
      localAnunciosData: this.anunciosData,
    };
  },
  computed: {
    anunciosFiltrados() {
      if (!this.localAnunciosData || !this.localAnunciosData.data) return [];

      // Asegurarse de que cada anuncio tenga la propiedad showFullDescription
      this.localAnunciosData.data.forEach(anuncio => {
        if (typeof anuncio.showFullDescription === 'undefined') {
          this.$set(anuncio, 'showFullDescription', false);
        }
      });

      if (!this.sidebarSearch) return this.localAnunciosData.data;

      const termino = this.sidebarSearch.toLowerCase();
      return this.localAnunciosData.data.filter(anuncio => {
  ...
  methods: {
    ...
    getCategoryItemStyle(category) {
      const theme = this.getCategoryTheme(category);
      return {
        background: `linear-gradient(90deg, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0.42) 100%), ${theme.gradient}`,
      };
    },
    fetchCategorias() {
    ...
    siguiente() {
      if (this.localAnunciosData.next_page_url) {
        window.location.href = this.localAnunciosData.next_page_url;
      }
    },
    previo() {
      if (this.localAnunciosData.prev_page_url) {
        window.location.href = this.localAnunciosData.prev_page_url;
      }
    },
    getLink(id) {
...
<style scoped>
  ...
  @keyframes logoPulse {
    0%, 100% {
      box-shadow: 0 0 0 0 rgba(255, 112, 176, 0.14);
    }
    50% {
      box-shadow: 0 0 0 6px rgba(255, 112, 176, 0);
    }
  }
  .contents-ctg {
  ...
  .categories-list li:hover .cat-badge {
